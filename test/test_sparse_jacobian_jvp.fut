...
-- output { true }
entry test_sparse_jvp_zero_pattern_matches_dense (x:[3]f64) : bool =
  let eps = 1e-9f64
  let jd = mask_with_pattern pat_ex3_zero (Dense.jac_dense_jvp f_ex3_zero x)
...
-- output { true }
entry test_sparse_jvp_ex4_matches_dense (x:[6]f64) : bool =
  let eps = 1e-9f64
  let jd = mask_with_pattern pat_ex4 (Dense.jac_dense_jvp f_ex4 x)
...
-- input  { [1.5f64, -2.0f64, 0.5f64, 7.0f64, 3.0f64, -4.0f64] }
-- output { true }
entry test_sparse_jvp_ex4_with_colors_matches_dense (x:[6]f64) : bool =
  let eps = 1e-9f64
  let jd = mask_with_pattern pat_ex4 (Dense.jac_dense_jvp f_ex4 x)
...
-- output { true }
entry test_sparse_jvp_ex5_matches_dense (x:[5]f64) : bool =
  let eps = 1e-9f64
...
-- input  { [1.5f64, -2.0f64, 0.5f64, 7.0f64, 3.0f64, -4.0f64] }
-- output { true }
entry test_prepared_jvp_ex4_matches_dense (x:[6]f64) : bool =
  let eps = 1e-9f64
  let jd = mask_with_pattern pat_ex4 (Dense.jac_dense_jvp f_ex4 x)
  let prepared = Sparse.prepare_jvp pat_ex4
...
-- input  { [1.5f64, -2.0f64, 0.5f64, 7.0f64, 3.0f64, -4.0f64] }
-- output { true }
entry test_prepared_jvp_reuse_two_points (x1:[6]f64) : bool =
  let eps = 1e-9f64
  ...
  let jd1 = mask_with_pattern pat_ex4 (Dense.jac_dense_jvp f_ex4 x1)
  let js1 = Sparse.eval_prepared_jvp_dense f_ex4 prepared x1

  let jd2 = mask_with_pattern pat_ex4 (Dense.jac_dense_jvp f_ex4 x2)
  let js2 = Sparse.eval_prepared_jvp_dense f_ex4 prepared x2
...
-- input  { [1.5f64, -2.0f64, 0.5f64, 7.0f64, 3.0f64, -4.0f64] }
-- output { true }
entry test_jvp_csr_from_csr_ex4_matches_dense (x:[6]f64) : bool =
  let eps = 1e-9f64
  ...
  let (out_row_offs, out_row_idx, vals) =
    Sparse.jac_jvp_csr_from_csr f_ex4 row_offs row_idx col_offs col_idx x
...
-- ==
-- entry: test_jvp_compressed_from_csr_ex4_matches_dense
-- input  { [1.5f64, -2.0f64, 0.5f64, 7.0f64, 3.0f64, -4.0f64] }
-- output { true }
entry test_jvp_compressed_from_csr_ex4_matches_dense (x:[6]f64) : bool =
  let eps = 1e-9f64
  ...
  let ((out_row_offs, out_row_idx), (_out_col_offs, _out_col_idx), colors, ys) =
    Sparse.jac_jvp_compressed_from_csr f_ex4 row_offs row_idx col_offs col_idx x

  let vals =
    Sparse.compressed_to_csr_vals out_row_offs out_row_idx colors ys
