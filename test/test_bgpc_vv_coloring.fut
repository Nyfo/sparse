...
def valid_coloring [m][n]
  (row_offs:[m+1]i64) (row_idx:[]i64) (colors:[n]i64) : bool =
...
-- ==
-- entry: test_bgpc_vv_ex1_exact
-- input  { }
-- output { true }
entry test_bgpc_vv_ex1_exact : bool =
  let (row_offs, row_idx) = CSR.csr_rows_from_pattern pat_ex1
  let (col_offs, col_idx) = CSR.csr_cols_from_pattern pat_ex1
  let colors = Col.vv_color_cols row_offs row_idx col_offs col_idx
...
-- ==
-- entry: test_bgpc_vv_star_exact
-- input  { }
-- output { true }
entry test_bgpc_vv_star_exact : bool =
